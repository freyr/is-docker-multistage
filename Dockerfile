...
FROM vendor as vendor-production
RUN --mount=type=secret,id=composer,target=/build/auth.json \
    composer install \
    --no-dev --no-interaction --prefer-dist --no-autoloader


FROM vendor as vendor-development
RUN --mount=type=secret,id=composer,target=/build/auth.json \
    composer install \
    --no-interaction --prefer-dist --no-autoloader


FROM os as app-common
EXPOSE 9000
WORKDIR /app
